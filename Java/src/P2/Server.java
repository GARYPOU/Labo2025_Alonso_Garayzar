...
public class Server {
    ...
    private static final HashMap<SocketAddress, Boolean> ackRecibidos = new HashMap<>();
    ...
    public static void verificarClienteRegistrado(String mensaje,DatagramPacket registro){
    ...
    public static void registrarCliente(DatagramSocket socketServidor,byte[] buffer ) throws IOException {
        ...
        while (clientes.size() < 2) {
            ...
            verificarClienteRegistrado(mensaje,registro);
        }
    }

    public static String recibirMensajeAgente(DatagramSocket socketServidor,byte[] buffer) throws IOException {
    ...
    public static void enviarMensajeClientes(DatagramSocket socketServidor, String mensaje){
        // Enviamos a todos los clientes registrados
        for (SocketAddress cliente : clientes) {
            new Thread(() -> {
                manejarCliente(socketServidor, mensaje, cliente);
            }).start();
        }
    }

    public static void verificarConfirmacionCliente(String ack,SocketAddress remitente){
    ...
    public static void esperandoConfirmacionCliente(DatagramSocket socketServidor) throws IOException {
        // Esperamos ACKs
        while (ackRecibidos.values().contains(false)) {
            ...
            verificarConfirmacionCliente(ack,remitente);
    ...
    public static void main(String[] args) {
        try {
            ...
            registrarCliente(socketServidor,buffer);
